...
using StudentApp.Migrations.SeedData;
...
namespace StudentApp.Tests
{
    public abstract class StudentAppTestBase : AbpIntegratedTestBase<StudentAppTestModule>
    {
        ...
        protected void UsingDbContext(Action<StudentAppDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<StudentAppDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<StudentAppDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<StudentAppDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<StudentAppDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<StudentAppDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<StudentAppDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<StudentAppDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<StudentAppDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<StudentAppDbContext, Task<T>> func)
        {
            T result;
